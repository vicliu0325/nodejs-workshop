...
router.post('/stocks/:stockId', authMiddleware.checkLogin, async (req, res, next) => {
  ...
  await memberModel.addFavoriteStock(req.session.member.id, req.params.stockId);
  // 再去拿一次資料
...
router.delete('/stocks/:stockId', authMiddleware.checkLogin, async (req, res, next) => {
  // 刪除 member_stocks 的表
  await memberModel.removeFavoriteStock(req.session.member.id, req.params.stockId);
  // 再去拿一次資料
  let stocks = await memberModel.getFavoriteStock(req.session.member.id);
  res.json({ message: 'ok', stocks });
});
